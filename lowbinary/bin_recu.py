...
class BinarizeConv2d(nn.Conv2d):
    ...
    def forward(self, input,num_bits=8, num_grad_bits=8):
        ...
        if math.isclose(self.tau.cpu().detach().item() ,1.):
        ...
        if num_bits != 0:
          ...
          weight_qparams = calculate_qparams(w2, num_bits=num_bits, flatten_dims=(1, -1),
                                            reduce_dim=None)
          w2 = quantize(w2, qparams=weight_qparams)

          a0 = self.quantize_input(a0, num_bits)
